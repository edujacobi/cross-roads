...
exports.run = async (bot, message, args) => {
	...
	const comunicado = new Discord.MessageEmbed()
		...
		.setFooter(`${message.author.id == bot.config.adminID ? `Administrador` : `Moderador`} ${bot.data.get(message.author.id, "username")}`, message.member.user.avatarURL())
		.setTimestamp()

	bot.users.fetch(id).then(user => user.send({
			embeds: [comunicado]
		})
		.catch(err => message.reply(`Não consegui mandar mensagem privada para ${bot.data.get(id, "username")} (${id})`)
			.catch(er => `Não consegui responder ${bot.data.get(id, "username")} nem no PV nem no canal. \`Comunicar\``)))
