...
exports.run = async (bot, message, args) => {
	...
	message.channel.send({
		...
		msg.react('➡️').then(msg.react('🆔')).catch(err => console.log("Não consegui reagir mensagem `topinvestidor`", err))
		...
		collector.on('collect', reaction => {
			if (msg) msg.reactions.removeAll().then(async () => {
				...
				}).catch(err => console.log("Não consegui editar mensagem `topinvestidor`", err))

				if (currentIndex !== 0)
					await msg.react('⬅️').catch(err => console.log("Não consegui reagir mensagem `topinvestidor`", err))
				if (currentIndex + 10 < top.length)
					msg.react('➡️').catch(err => console.log("Não consegui reagir mensagem `topinvestidor`", err))
				msg.react('🆔').catch(err => console.log("Não consegui reagir mensagem `topinvestidor`", err))
			}).catch(err => console.log("Não consegui remover as reações mensagem `topinvestidor`", err))
		})
		collector.on('end', reaction => {
			if (msg) msg.reactions.removeAll().catch(err => console.log("Não consegui remover as reações mensagem `topinvestidor`", err))
		})
	}).catch(err => console.log("Não consegui enviar mensagem `topinvestidor`", err))
};
//--
exports.config = {
