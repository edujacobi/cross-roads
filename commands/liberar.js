...
exports.run = async (bot, message, args) => {
	...
	if (!alvo)
		return bot.createEmbed(message, `Caralho, ${await bot.data.get(message.author.id + ".username")}, escolha um usuário`)

	if (alvo < 0 || (alvo % 1 != 0) || alvo.toString().length != 18)
		return bot.createEmbed(message, `Caralho, ${await bot.data.get(message.author.id + ".username")}, o usuário é inválido`)
	...
	if (option == 'prisao') {
		...
		let uGalo = await bot.galos.get(alvo)
		uGalo.emRinha = false
		await bot.galos.set(alvo, uGalo)
