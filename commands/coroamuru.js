...
exports.run = async (bot, message, args) => {
	...
	let row = new Discord.MessageActionRow()
		.addComponents(button).addComponents(buttonTop);;

	await message.channel.send({
		components: [row],
		embeds: [embed]
	}).catch(err => console.log("Não consegui enviar mensagem `coroamuru`")).then(msg => {

		const filter = (button) => ['attack', 'top'].includes(button.customId)
		...
		collector.on('collect', async btn => {
			if (btn.customId === 'attack') {
				...
				if (uGalo.train == 1) {
					if (uGalo.trainTime > currTime)
						return bot.createEmbed(message, `${uData.username}, seu galo está treinando por mais ${bot.segToHour((uGalo.trainTime - currTime) / 1000)} ${coroamuru}`, null, bot.colors.white)
					else
						...
						setTimeout(() => {
							user.send(`Você pode atacar o Coroamuru novamente! ${coroamuru}`)
								.catch(err => message.reply(`você pode atacar o Coroamuru novamente! ${coroamuru}`)
									.catch(er => `Não consegui responder ${bot.data.get(btn.user.id, "username")} nem no PV nem no canal. \`Coroamuru\``))
				...
				if (vidaAtual <= 0) {
					...
					let uDataWin = bot.data.get(topDano.id)

					uDataWin.moni += premio
					bot.data.set(topDano.id, uDataWin)
					

					bot.users.fetch(topDano.id).then(user => {
						user.send({
							embeds: [embedWin]
						})
					})
			...
			\nEle não mais absorve energia de todos os jogadores, mas ainda procura pelo seu objetivo neste novo mundo!`)
				...
				msg.edit({
					embeds: [embed2],
					components: [row],
				}).catch(err => console.log("Não consegui editar mensagem `coroamuru`"))
				...
				message.channel.send({
					embeds: [embedATK]
				}).catch(err => console.log("Não consegui enviar mensagem `coroamuru`"))

			} else if (btn.customId === 'top' && btn.user.id === message.author.id) {
				// collector.stop()
