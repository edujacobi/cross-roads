...
exports.run = async (bot, message, args) => {
	...
	for (let [id, user] of bot.data) {
		if (user.username != undefined && user.prisaoGastos > 0) {
	...
	const generateEmbed = start => {
		...
		if (top.length > 0) {
			...
			if (userComando) {
				...
				let emote = user.classe ? bot.guilds.cache.get('798984428248498177').emojis.cache.find(emoji => emoji.id == bot.classes[user.classe].emote) : `<:Inventario:814663379536052244>`
				topGlobalString += `\`${i + 1}.\` ${emote} __**${user.username}**__ R$ ${user.prisaoGastos.toLocaleString().replace(/,/g, ".")}\n`;
			}
	...
	message.channel.send({
		...
		msg.react('➡️').then(msg.react('🆔')).catch(err => console.log("Não consegui reagir mensagem `topsuborno`"))
		...
		collector.on('collect', reaction => {
			if (msg) msg.reactions.removeAll().then(async () => {
				...
				}).catch(err => console.log("Não consegui editar mensagem `topsuborno`"))

				if (currentIndex !== 0)
					await msg.react('⬅️').catch(err => console.log("Não consegui reagir mensagem `topsuborno`"))
				if (currentIndex + 10 < top.length)
					msg.react('➡️').catch(err => console.log("Não consegui reagir mensagem `topsuborno`"))
				msg.react('🆔').catch(err => console.log("Não consegui reagir mensagem `topsuborno`"))
			}).catch(err => console.log("Não consegui remover as reações mensagem `topsuborno`"))
		})
		collector.on('end', reaction => {
			if (msg) msg.reactions.removeAll().catch(err => console.log("Não consegui remover as reações mensagem `topsuborno`"))
		})
	}).catch(err => console.log("Não consegui enviar mensagem `topsuborno`"))
};
//--
exports.config = {
