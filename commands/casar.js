...
exports.run = async (bot, message, args) => {
    ...
    if (!option && !target) {
        ...
        const embed = new Discord.MessageEmbed()
            ...
            .addField("💔 Divórcio", `Caso o casamento fique com um nível muito baixo, os efeitos positivos ficarão desativados até o nível aumentar. Você pode forçar um divórcio, mas custará caro.`)
        ...
        let msg = await message.channel.send({
            components: [row],
            embeds: [embed]
        }).catch(err => console.log("Não consegui enviar mensagem `casar`"))
        ...
        collector.on('collect', async b => {
            ...
            if (b.customId == message.id + message.author.id + 'anel') {
                msg.edit({
                    ...
                    embeds: [embed]
                }).catch(err => console.log("Não consegui editar mensagem `casar`"));
                ...
                if (uData.anel != null)
                    return bot.createEmbed(message, `Você já possui um 💍 Anel!`, null, bot.colors.casamento)
    ...
    if (target) {
        ...
        if (uData.anel == null)
            return bot.createEmbed(message, `${emote} Você não possui um 💍 Anel! Compre um para fazer um pedido de casamento!`, null, bot.colors.casamento)
        if (tData.anel == null || tData.anel != uData.anel)
            return bot.createEmbed(message, `${emote} Seu parceiro não possui um 💍 Anel igual ao seu! Vocês devem possui o mesmo tipo de anel`, null, bot.colors.casamento)
        ...
        collectorAceito.on('collect', async b => {
            ...
            if (b.customId == message.id + message.author.id + 'sim') {
                ...
                if (uData.anel == null)
                    return bot.createEmbed(message, `Você não possui um 💍 Anel! Compre um para fazer um pedido de casamento`, null, bot.colors.casamento)
                if (tData.anel == null || tData.anel != uData.anel)
                    return bot.createEmbed(message, `Seu parceiro não possui um 💍 Anel igual ao seu! Vocês devem possui o mesmo tipo de anel`, null, bot.colors.casamento)
                ...
                bot.casais.ensure(bot.casais.size, casamento)
                ...
                msg.edit({
                    ...
                    .then(() => msg.react('🎉')).catch(err => console.log("Não consegui reagir mensagem `casar`"))
