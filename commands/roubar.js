...
Você já roubou \`${uData.roubosW.toLocaleString().replace(/,/g, ".")}\` vezes, falhou \`${uData.roubosL.toLocaleString().replace(/,/g, ".")}\` vezes e foi roubado \`${uData.qtRoubado.toLocaleString().replace(/,/g, ".")}\` vezes`)
	...
	async function roubo() {
		...
		// 	console.log('retornoRoubo', retornoRoubo)
		//
		// 	if (!retornoRoubo)
		// 		return message.reply({embeds: [embeds.erro]})
		//
		// 	console.log(retornoRoubo)
		//
		// 	return retornoRoubo
		// })
		...
		setTimeout(async () => {
			...
			// console.log('final', retornoRoubo)
			//
			// defPower = retornoRoubo.defPower
			// chance_espancar = retornoRoubo.chance_espancar
			// tempo_preso = retornoRoubo.tempo_preso
