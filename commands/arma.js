	...
	if (!args[0]) {
		...
		Object.values(bot.guns).forEach(arma => {
			...
			if (arma.atk || arma.def)
				embed.addField(`${bot.config[arma.emote]} ${arma.desc}`, `${arma.atk ? arma.atk : '0'} / ${arma.def ? arma.def : '0'}`, true)
			else {
				if (arma.desc == 'Celular')
					embed.addField(`${bot.config[arma.emote]} ${arma.desc}`, `\`;celular\``, true)
				if (arma.desc == 'Jetpack')
					embed.addField(`${bot.config[arma.emote]} ${arma.desc}`, `Para fugir`, true)
			}
		})
