...
module.exports = (bot) => {
	...
	bot.msgTrabalhando = (message, uData, username) => {
		...
		if (minutes < 0)
			return bot.createEmbed(message, `${username ?? 'Você'} deve receber seu salário antes de fazer isto ${bot.config.trabalhando}`, null, 'YELLOW')

		return bot.createEmbed(message, `${username ?? 'Você'} está trabalhando por mais ${bot.segToHour(minutes)} e não pode fazer isto ${bot.config.trabalhando}`, bot.jobs[uData.job].desc, 'YELLOW')
	}
